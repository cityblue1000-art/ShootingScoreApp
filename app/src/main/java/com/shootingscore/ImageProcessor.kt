...
class ImageProcessor(private val detector: YoloDetector) {
    ...
    fun processWithReference(bitmap: Bitmap, reference: Bitmap): ProcessingResult {
        ...
        fun createMat(): Mat = Mat().also { mats.add(it) }

        try {
            val currentMat = createMat()
            Utils.bitmapToMat(bitmap, currentMat)

            val warpedCurrentMat = warpTarget(bitmap) ?: return ProcessingResult(bitmap, emptyList(), 0, false)
            mats.add(warpedCurrentMat)
            
            val resBitmap = Bitmap.createBitmap(targetSize.toInt(), targetSize.toInt(), Bitmap.Config.ARGB_8888)
            Utils.matToBitmap(warpedCurrentMat, resBitmap)

            val warpedRef = createMat()
            if (reference.width == targetSize.toInt() && reference.height == targetSize.toInt()) {
                Utils.bitmapToMat(reference, warpedRef)
            } else {
                val tempRef = warpTarget(reference) ?: return ProcessingResult(resBitmap, emptyList(), 0, false)
                tempRef.copyTo(warpedRef)
                tempRef.release()
            }

            val grayCurrent = createMat()
            val grayRef = createMat()
            Imgproc.cvtColor(warpedCurrentMat, grayCurrent, Imgproc.COLOR_RGBA2GRAY)
            Imgproc.cvtColor(warpedRef, grayRef, Imgproc.COLOR_RGBA2GRAY)

            val alignedCurrent = createMat()
            val tempAligned = refineAlignment(grayRef, grayCurrent)
            tempAligned.copyTo(alignedCurrent)
            tempAligned.release()

            val smoothCurrent = createMat()
            val smoothRef = createMat()
            ...
            val inkMask = createMat()
            Imgproc.threshold(smoothRef, inkMask, 175.0, 255.0, Imgproc.THRESH_BINARY_INV)
            
            val inkKernel = Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, Size(9.0, 9.0))
            Imgproc.dilate(inkMask, inkMask, inkKernel)
            inkKernel.release()

            val inkMaskInv = createMat()
            Core.bitwise_not(inkMask, inkMaskInv)

            val cleanDiff = createMat()
            Core.bitwise_and(diff, diff, cleanDiff, inkMaskInv)

            val totalDiff = Core.sumElems(cleanDiff).`val`[0]
            if (totalDiff > 15000000.0) {
                return ProcessingResult(resBitmap, emptyList(), 0, true)
            }

            val maskedDiff = createMat()
            val circleMask = createMat()
            Imgproc.circle(circleMask, Point(320.0, 320.0), 310, Scalar(255.0), -1)
            cleanDiff.copyTo(maskedDiff, circleMask)

            val thresholdVal = (Core.mean(maskedDiff).`val`[0] * 4.2).coerceIn(55.0, 110.0)
            ...
            Imgproc.morphologyEx(maskedDiff, maskedDiff, Imgproc.MORPH_OPEN, morphKernel)
            morphKernel.release()

            val contours = mutableListOf<MatOfPoint>()
            Imgproc.findContours(maskedDiff, contours, Mat().also { it.release() }, Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_SIMPLE)
            ...
            Log.e("ImageProcessor", "Error in processWithReference: ${e.message}")
    ...
    fun processWithAutoAlign(bitmap: Bitmap, reference: Bitmap?): ProcessingResult {
        ...
        try {
            ...
            val thresholdVal = (Core.mean(maskedDiff).`val`[0] * 4.2).coerceIn(50.0, 110.0)
            Imgproc.threshold(maskedDiff, maskedDiff, thresholdVal, 255.0, Imgproc.THRESH_BINARY)
            ...
            Log.e("ImageProcessor", "Error in processWithAutoAlign: ${e.message}")
            return ProcessingResult(bitmap, emptyList(), 0, false)
        } finally {
            mats.forEach { it.release() }
        }
